        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {error && (
            <motion.p
              ...
              className="text-sm text-yellow-600 font-semibold mb-6"
            >
              {error}
            </motion.p>
          )}
          {loading && (
            <motion.p
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              className="text-sm text-gray-500 font-semibold mb-6"
            >
              Loading the latest room catalog...
            </motion.p>
          )}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {featuredRooms.map((room, index) => (
              ...
              >
                <div className="h-64 bg-cover bg-center" style={{ backgroundImage: `url(${room.image_url})` }} />
                <div className="p-6">
                  <h3 className="text-2xl font-semibold text-green-800 mb-2">{room.name}</h3>
                  <p className="text-3xl font-bold text-yellow-600 mb-4">{formatCurrency(room.price_per_night)}</p>
                  <p className="text-gray-600 mb-3 text-lg">{room.description}</p>
                  <div className="mb-4">
                    <p className="text-sm text-gray-400">Occupancy: {room.occupancy} guests</p>
                    <p className="text-sm text-gray-400">Bed: {room.bed_type}</p>
                  </div>
